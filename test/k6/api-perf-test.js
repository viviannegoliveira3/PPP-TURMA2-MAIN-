...
// lightweight local random name generator to avoid external CDN dependency
function randomName() {
  const first = ['Ana','Bruno','Carla','Diego','Eduardo','Fernanda','Gustavo','Helena','Igor','Julia'];
  const last = ['Silva','Souza','Oliveira','Santos','Pereira','Costa','Almeida','Gomes','Ribeiro','Martins'];
  return `${first[Math.floor(Math.random()*first.length)]} ${last[Math.floor(Math.random()*last.length)]} ${Math.floor(Math.random()*10000)}`;
}
// simple local options for quick local runs; replace or import a centralized options file for CI
